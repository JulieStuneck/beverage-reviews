...
public class JpaMappingTest {
	...
	public void shouldSaveandLoadCategory() {
		...
		// Optional = is it there or isn't it there (Spring command)
		Optional<Category> result = categoryRepo.findById(coffeeId);
		coffee = result.get();
		assertThat(coffee.getBeverage(), is("coffee"));
		assertThat(result, is(Optional.of(coffee))); // need either of these tests for Optional
	...
	public void shouldGenerateCategoryId() {
		...
		assertThat(coffeeId, is(greaterThan(0L))); // run with 0 and also 100 to make sure is working
	}
	...
	public void shouldEstablishReviewToCategoryRelationship() {
		...
		Optional<Review> result = reviewRepo.findById(dunkinId);
		dunkin = result.get(); // this will generate creation of "Hashcode and Equals" for Entities' id's
	...
	@Test
	public void shouldEstablishReviewToTag() {
		Category coffee = categoryRepo.save(new Category("coffee"));		
		Tag hot = tagRepo.save(new Tag("hot"));
